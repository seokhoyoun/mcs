...
namespace Nexus.Infrastructure.Persistence.Redis
{
    public class RedisLotRepository : ILotRepository
    {
        ...
        public async Task<bool> DeleteAsync(string id, CancellationToken cancellationToken = default)
        {
            ...
            {
                string lotStepIdsJson = GetHashValue(lotHashEntries, "lot_step_ids");
        ...
        {
            RedisValue cassetteIdsJson = hashEntries.FirstOrDefault(e => e.Name == "cassette_ids").Value;
            List<string>? cassetteIds = JsonSerializer.Deserialize<List<string>>(cassetteIdsJson!);
            ...
            string lotStepIdsJson = GetHashValue(hashEntries, "lot_step_ids");
            if (!string.IsNullOrEmpty(lotStepIdsJson))
            {
                List<string> lotStepIds = JsonSerializer.Deserialize<List<string>>(lotStepIdsJson) ?? new List<string>();
                foreach (string stepId in lotStepIds)
        ...
        private async Task<LotStep> ConvertHashToLotStepAsync(string id, HashEntry[] hashEntries, CancellationToken cancellationToken = default)
        {
            ...
            // Cassette 객체들 로드
            string cassetteIdsJson = GetHashValue(hashEntries, "cassette_ids");
            if (!string.IsNullOrEmpty(cassetteIdsJson))
            {
                List<string> cassetteIds = JsonSerializer.Deserialize<List<string>>(cassetteIdsJson) ?? new List<string>();
            ...
            // PlanGroup 객체들 로드
            string planGroupIdsJson = GetHashValue(hashEntries, "plan_group_ids");
            if (!string.IsNullOrEmpty(planGroupIdsJson))
            {
                List<string> planGroupIds = JsonSerializer.Deserialize<List<string>>(planGroupIdsJson) ?? new List<string>();
        ...
        private async Task<Cassette?> GetCassetteByIdAsync(string cassetteId, CancellationToken cancellationToken = default)
        {
            ...
            string cassetteName = GetHashValue(hashEntries, "name");

            // Tray 목록 로드
            string trayIdsJson = GetHashValue(hashEntries, "tray_ids");
        ...
        private async Task<PlanGroup?> GetPlanGroupByIdAsync(string planGroupId, CancellationToken cancellationToken = default)
        {
            ...
            string planGroupName = GetHashValue(hashEntries, "name");
            string groupTypeStr = GetHashValue(hashEntries, "group_type");
